...
class SduBkjws(object):
    ...
    @staticmethod
    def _get_current_xnxq():
        """
        根据当前日期自动计算学年学期

        :return: 学年学期字符串，格式如 ``2016-2017-2``
        :rtype: str
        """
        now = time.localtime()
        year = now.tm_year
        month = now.tm_mon
        if month >= 8:
            return '{}-{}-1'.format(year, year + 1)
        elif month == 1:
            return '{}-{}-1'.format(year - 1, year)
        elif 2 <= month <= 7:
            return '{}-{}-2'.format(year - 1, year)
        else:
            return '{}-{}-1'.format(year, year + 1)

    def comment_lesson(self, xnxq=None, scores=None, comment_text=None):
        """
        完成教学评估（评教），自动给所有未评教的课程打分和评价

        :param xnxq: 学年学期，格式如 ``2016-2017-2``，默认自动计算当前学期
        :type xnxq: str
        :param scores: 各指标评分，dict 格式 ``{zbid: value}``，
                       如 ``{36: 5.0, 52: 4.0}``
                       不传则默认所有题打 5.0 分，主观题选 "推荐"
        :type scores: dict
        :param comment_text: 主观评语（对应 zbid=57），不传则随机选择一条默认评语
        ...
        default_comments = [
            ...
            '老师长得很好看呀~~最喜欢他的课了~~',
            '这个老师是我的老乡==',
            '这是我见过的PPT做的最用心的老师,给老师点个赞',
            '老师讲课突出重点，内容详细，条理清晰，细致入微',
            '老师很关心我们~~'
        ]

        if xnxq is None:
            xnxq = self._get_current_xnxq()

        if comment_text is None:
            comment_text = random.choice(default_comments)

        # 默认评分：选择题 (zbid 36-55) 全 5.0，主观选择 (zbid 56) 为 "推荐"
        if scores is None:
            scores = {}
            for zbid in range(36, 56):
                scores[zbid] = 5.0
            scores[56] = '推荐'

        # 获取需要评教的课程，过滤出未评教的 (pgcs == 0)
        course_list = self.get_comment_lesson_info()
        unevaluated = [c for c in course_list if c.get('pgcs', 1) == 0]
        ...
        for course in unevaluated:
            parts = [
                'xnxq={}'.format(xnxq),
                'kch={}'.format(course['kch']),
                'jsh={}'.format(course['jsh']),
                'wjid=1',
                'wjmc=山东大学课堂教学评价',
            ]

            # 选择题 zbid 36-55，对应 zbda_0 ~ zbda_19
            for i, zbid in enumerate(range(36, 56)):
                parts.append('zbid={}'.format(zbid))
                parts.append('zblx=选择')
                parts.append('zbda_{}={}'.format(i, scores.get(zbid, 5.0)))

            # 主观选择 zbid 56，zbda_20
            parts.append('zbid=56')
            parts.append('zblx=主观选择')
            parts.append('zbda_20={}'.format(scores.get(56, '推荐')))

            # 主观评语 zbid 57，zbda_21
            parts.append('zbid=57')
            parts.append('zblx=主观')
            parts.append('zbda_21={}'.format(comment_text))
            ...
            try:
                ...
                if isinstance(response, dict):
                    ...
                    success = 'success' in str(response)
                results.append({
                    'kch': course['kch'],
                    'kcm': course.get('kcm', ''),
                    'jsm': course.get('jsm', ''),
